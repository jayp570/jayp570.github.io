...
function makeMap() {
    ...
    for(let i = 0; i < mapTemplate.length; i++) {
        ...
        for(let j = 0; j < mapTemplate[i].length; j++) {
            ...
            if(mapTemplate[i][j] == 6) {
                lights.push(new Tile(j*TILESIZE, i*TILESIZE, mapTemplate[i][j]));
            }
        }
    }
}
                ...
                for(let k = 0; k < map[j].length; k++) {
                    if(map[j][k].num > 0) {
                        if(map[j][k].checkCollision(pos.x, pos.y, 1, 1)) {
                            ...
                            let lighting = 0;
                            for(let lightCount = 0; lightCount < lights.length; lightCount++) {
                                
                            }
                            
                            lengths.push(   {
                                    "distance": length*Math.cos(rayAngle-playerLookAngle),
                                    "lighting": lighting,
                                    "red": map[j][k].red,
                                    "blue": map[j][k].blue,
                                    "green": map[j][k].green
...
function drawScreen() {
    ...
    for(let i = 0; i < lengths.length; i++) {
        let color = mixColors([lengths[i].red, lengths[i].green, lengths[i].blue], [0, 0, 0], lengths[i].distance*brightness/350, true)
